    ...
    */
    ...
    EventHelper.prototype.fail = function (errorMap) {
        ...
        let wrapper = function (eventType, ...args) {
            ...
            if(arguments.length==0){
                handler = function () { throw new Error(...args) };
            }else {
                handler = errorMap[eventType] || function () { throw new Error(...args) };
            }
            handler.apply(that, args);
        };
    ...
    EventHelper.prototype.after = function (eventType, times, handler) {
        ...
        let wrapper = function (result) {
            count++;
            if (typeof result == 'object' && result.type == 'group' && result.hasOwnProperty('index')) {
