!(function (name, definition) {
    ...
    */
    ...
    EventHelper.prototype.concurrent = function(eventType, limit, asyncHandler, asyncParams){
        ...
        let asyncList = asyncParamsClone.splice(0, limit).map(param => asyncHandler(params, that.group(eventType)));
        let indexMap = {};
        let resultArr = [];
        let dataIndex, nextParam, indexValue, handler;
        ...
        handler = (data)=> {
            if(typeof data == 'object' && data.type == 'group' && data.hasOwnProperty('index')){
                ...
                asyncList.splice(indexMapValue, 1, asyncHandler(nextParam, that.group(eventType)));
                indexMap[asyncParams.findIndex(nextParam)] = indexMapValue;
                resultArr[dataIndex] = data.result;
                //if the last done , then fire 'finish' event
                if(resultArr.length == asyncParams.length){
                    that.emit('finish', resultArr);
                }
            }
        };
